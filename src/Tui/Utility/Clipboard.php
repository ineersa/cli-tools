...
final class Clipboard
{
    public static function copy(string $text): bool
    {
        ...
        if (\PHP_OS_FAMILY === 'Darwin' && self::runPipe(['pbcopy'], $text)) return true;
        if (\PHP_OS_FAMILY === 'Windows' && self::runPipe(['cmd', '/c', 'clip'], $text)) return true;

        if (self::commandExists('wl-copy') && self::runPipe(['wl-copy'], $text)) return true;
        if (self::commandExists('xclip') && self::runPipe(['xclip', '-selection', 'clipboard'], $text)) return true;
        if (self::commandExists('xsel') && self::runPipe(['xsel', '--clipboard', '--input'], $text)) return true;
        ...
        if (getenv('TMUX')) {
            ...
            if (self::runPipe(['tmux', 'load-buffer', '-'], $text)) {
                ...
                if (self::osc52ToStdout($text, true)) return true;
        ...
        if (self::osc52ToStdout($text, false)) return true;
